...
<input type="text" id="property" value='<?php echo json_encode($property) ?>'>
<input type="text" id="member_name" value='<?php echo json_encode($family) ?>'>
...
$(document).ready(function(){
	...
	$.each(family,function(key,val){
	   	...
	   	{
	        select_family = select_family+'<option value="'+val.member_name+'">'+val.member_name+'</option>';
	    }
	    else
	    {
	      	select_family = select_family+'<option value="'+val.member_name+'">'+val.member_name+'</option>';
	...
	select_family = select_family+'</select>';
   	...
   	var input = '<input type="number" class="number" placeholder="allocation in %">';
   	var my_div = '<div class="show_more">' + select_property + select_family + input + '<button class="add_more">OK</button></div>';
   	$('.container').append(my_div);
   	//console.log(select_family);
	});
});
